...
/**
 * Test for [LoadLastRecipesUseCase]
 */
@ExperimentalCoroutinesApi
class LoadLastRecipesUseCaseTest {
    ...
    fun loadRecipes_receiveUnorderedList_resultRecipesOrderedById() = runTest {
        val useCase = LoadLastRecipesUseCase(
        ...
        assertEquals(result.data?.toString(), TestData.foodDish3.id, result.data?.recipes?.first()?.id)
        assertEquals(TestData.foodDish2.id, result.data?.recipes?.get(1)?.id)
    }
